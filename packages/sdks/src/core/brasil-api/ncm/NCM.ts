...
export class NCM {
  ...
  public async getNCMs() {
    ...
    const response = await request<NCMInfo[]>(`${BRASIL_API_URL}/ncm/v1`, {
      method: "GET",
    });
  ...
  public async searchNCMs(search: string) {
    ...
    const response = await request<NCMInfo[]>(
      `${BRASIL_API_URL}/ncm/v1?search=${search}`,
      {
        method: "GET",
      },
    );
