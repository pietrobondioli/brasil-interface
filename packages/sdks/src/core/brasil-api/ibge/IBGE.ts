...
export class IBGE {
  ...
  public async getMunicipios(uf: string, providers: string) {
    ...
    // );
    const response = await request<Municipio[]>(
      `${BRASIL_API_URL}/ibge/municipios/v1/${uf}?providers=${providers}`,
      {
        method: "GET",
      },
    );
  ...
  public async getEstado(code: string) {
    ...
    const response = await request<Estado>(
      `${BRASIL_API_URL}/ibge/uf/v1/${code}`,
      {
        method: "GET",
      },
    );
