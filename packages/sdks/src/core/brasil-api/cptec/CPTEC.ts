...
export class CPTEC {
	private readonly http = new FetchJson(`${BRASIL_API_URL}/CPTEC/v1`);
	...
	public async getCidades() {
		const response = await this.http.get<any>("cidade");
	...
	public async buscarCidades(cityName: string) {
		const response = await this.http.get<any>(`cidade/${cityName}`);
	...
	public async getClimaAeroporto(icaoCode: string) {
		const response = await this.http.get<any>(`clima/aeroporto/${icaoCode}`);
	...
	public async getPrevisaoMeteorologica(cityCode: number) {
		const response = await this.http.get<any>(`clima/previsao/${cityCode}`);
	...
	public async getPrevisaoOceanica(cityCode: number) {
		const response = await this.http.get<any>(`ondas/${cityCode}`);
